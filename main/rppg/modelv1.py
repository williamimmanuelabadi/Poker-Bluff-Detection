...
class TSCAN(nn.Module):
    ...
    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
                 dropout_rate2=0.5, pool_size=(2, 2), nb_dense=128, frame_depth=1, img_size=72):
    ...
    def forward(self, inputs, params=None):
        ...
        inputs = self.TSM_1(diff_input)
        inputs = torch.tanh(self.motion_conv1(inputs))
        inputs = self.TSM_2(inputs)
        inputs = torch.tanh(self.motion_conv2(inputs))

        r = torch.tanh(self.apperance_conv1(raw_input))
        ...
        inputs = torch.tanh(self.motion_conv3(inputs))
        inputs = self.TSM_4(inputs)
        inputs = torch.tanh(self.motion_conv4(inputs))

        r = torch.tanh(self.apperance_conv3(r))
        r = torch.tanh(self.apperance_conv4(r))

        g = torch.sigmoid(self.apperance_att_conv2(r))
...
class EfficientPhys_residualTSM(nn.Module):

    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
                 dropout_rate2=0.5, pool_size=(2, 2), nb_dense=128, frame_depth=180, img_size=72, channel='raw'):
        super(EfficientPhys_residualTSM, self).__init__()
        self.in_channels = in_channels
        self.kernel_size = kernel_size
        self.dropout_rate1 = dropout_rate1
        self.dropout_rate2 = dropout_rate2
...
class resNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Linear(30720, 24, bias=True)
